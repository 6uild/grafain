...
This is a script for generating Go code https://github.com/iov-one/weave/blob/master/cmd/grafaincli/cmd_gov.go#L51-L60
It saves a lot of time (and bugs) creating type casting and conversion for messages used by grafain transactions.

```bash
#!/bin/bash

# Copy this directly from the ProposalOptions defined in cmd/grafain/app/codec.proto
...
# Copy this directly from the ExecuteProposalBatchMsg defined in cmd/grafain/app/codec.proto
# Remove all comment lines (starts with //)
proposalbatch="
...
while read -r m; do
	...
	then
		echo "	case *grafain.ExecuteBatchMsg:"
		echo "		msgs, err := msg.MsgList()"
		echo "		if err != nil {"
		echo "			return fmt.Errorf(\"cannot extract messages: %s\", err)"
		echo "		}"
		echo "		var messages []grafain.ExecuteProposalBatchMsg_Union"
		...
		while read -r m; do
			...
			echo "			case *$tp:"
			echo "				messages = append(messages, grafain.ExecuteProposalBatchMsg_Union{"
			echo "					Sum: &grafain.ExecuteProposalBatchMsg_Union_$name{"
			echo "						$name: m,"
			echo "					},"
			echo "				})"
		done <<< $proposalbatch
		...
		echo "			}"
		echo "		}"
		echo "		option.Option = &grafain.ProposalOptions_ExecuteProposalBatchMsg{"
		echo "			ExecuteProposalBatchMsg: &grafain.ExecuteProposalBatchMsg{"
	...
	echo "	case *$tp:"
	echo "		option.Option = &grafain.ProposalOptions_$name{"
	echo "				$name: msg,"
	echo "		}"
done <<< $proposals
*/
