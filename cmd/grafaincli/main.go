...
// commands is a register of all available commands that can be executed by
// this program. The name is used to match with the first argument given.
//
// When a cmd function is called it is given stdin, stdout and command line
...
//
// A command function is an independent runnable that is taking input and output
...
// pipeline line:
//
//   $ grafaincli release-escrow -escrow 1 \
//       | grafaincli as-proposal \
//       | grafaincli sign \
//       | grafaincli submit
//
var commands = map[string]func(input io.Reader, output io.Writer, args []string) error{
	...
	"del-artifact":              cmdDeleteArtifact,
...
func main() {
	if len(os.Args) == 1 {
		fmt.Fprintf(os.Stderr, "%s is a command line client for the GRAFAIN application.\n\n", os.Args[0])
